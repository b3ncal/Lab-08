...
missing_values_RainTomorrow <- sum(is.na(weather$RainTomorrow))


if (missing_values_RainTomorrow > 0) {
...
rainy_days_by_city <- weather_noNA %>%
  group_by(Location) %>%
  summarise(Prop_RainyDays = mean(RainToday == "Yes"))

rainy_days_by_city <- rainy_days_by_city %>%
  arrange(desc(Prop_RainyDays))

print(rainy_days_by_city)
```
